...
\author{Jianxu Wang, Huijun Zhou, and Xuewei Cheng
...
\begin{itemize}
\item \textbf{Prediction head $f(\cdot)$}: A 2-layer MLP (768$\to$512$\to$2) with BatchNorm, ReLU, and Dropout, producing per-class logits.
\item \textbf{Selection head $g(\cdot)$}: A 2-layer MLP (768$\to$512$\to$1) with BatchNorm and ReLU, followed by a sigmoid to produce a scalar confidence score $g(\cdot) \in [0,1]$. Following SelectiveNet \cite{geifman2019selectivenet}, the final layer bias is initialized to $2.0$ to start training with high coverage. The score is thresholded to implement abstention: if $g(\cdot) \ge \tau$, the prediction is accepted; otherwise, the model abstains.
\item \textbf{Auxiliary head} $h(\cdot)$: A 2-layer MLP (768$\to$512$\to$2) with BatchNorm, ReLU, and Dropout, trained on the standard cross-entropy objective to stabilize representation learning.
\end{itemize}

We formulate a differentiable objective to approximate the constraints in Theorem \ref{Th2}. Per-sample cross-entropy losses $\ell_i$ are computed with \texttt{reduction='none'} \cite{geifman2019selectivenet}. To enforce a target coverage $c$, we introduce a quadratic penalty $\Psi(g) = \beta \max(0, c - \hat{\Phi}_n)^2$, where $\beta$ controls the constraint weight. Starting from the bag-level representation $\mathbf{z} \in \mathbb{R}^d$ (Subsection \ref{S3-1-3}), the overall loss is:
\begin{equation*}
    \mathcal{L} = \gamma \left(\hat{R}_n(f,g) + \Psi(g)\right) + (1-\gamma) \, \ell_{\mathrm{ce}}(h(\mathbf{z}_i), y_i),
\end{equation*}
where $\gamma$ balances selective optimization and auxiliary classification. Minimizing $\mathcal{L}$ drives the model to accept samples with low prediction error. We then introduce four cases to achieve explicit control over risk and/or false negative rate (FNR) as illustrated in Figure \ref{F1}.
...
We adopt an image-level stratified 5-fold cross-validation, with an 80\%/20\% training/test split per fold. In addition to standard metrics (AUC and accuracy), we report ROC/PR curves and conduct calibration analysis using reliability diagrams and Brier scores \cite{guo2017calibration}. Unless stated otherwise, results are reported using the final training checkpoint with a default decision threshold $\tau = 0.5$. To ensure reproducibility, we fix random seeds and enable deterministic cuDNN. 
...
Empirically calibrated thresholds $\hat{\tau}$ are derived for three target risk levels: $10\%$, $5\%$, and $1\%$. The resulting score distributions and prediction performance are summarized in Figure~\ref{fig:np_summary} and Table~\ref{tab:table3}.
...
We further evaluated the selective inference framework on the external dataset and Table~\ref{tab2} summarizes the results. Case~3 (risk control) achieves well-calibrated risk on the external data: at the $5\%$ target, the test risk is $4.87\%$ with $61.19\%$ coverage. Case~4, with explicit FNR control, reduces false negatives more aggressively. At the $5\%$ target FNR, the external FNR reaches $3.20\%$—well below the bound—with $56.70\%$ coverage and $95.72\%$ accuracy, outperforming its Case~3 counterpart in sensitivity. These trends broadly mirror those on the local validation set (Tables~\ref{tab:case3_results} and~\ref{tab:case4_results}), though coverage on the external data is moderately lower, reflecting the increased domain shift. Overall, the framework maintains consistent selective classification behavior across datasets.
