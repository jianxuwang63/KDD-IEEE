...
\textcolor{red}{Overall comments on the first paragraph:I suggest adding several literature citations to the first and second sentences of the first paragraph to strengthen the academic basis and persuasiveness of the statements.\\
The second section elaborates the challenges of MIP. Accordingly, I recommend supplementing relevant content in the first paragraph to clarify that MIP accounts for only a small proportion among cancer patients, making this task an inherently imbalanced classification problem.I believe the main issues in Chapter 1 lie in the first two paragraphs. The subsequent content contains only one minor problem, which has already been marked in red in the original text. I have also placed the revised version of the first two paragraphs in the draft file.}
...
While the baseline model follows a standard training objective that minimizes average classification error, this approach overlooks the clinical asymmetry of risks: false negatives (FN) are considerably more costly than false positives (FP) in high‑stakes diagnostic settings. To incorporate this asymmetry directly into representation learning, \textcolor{red}{we adopt} a cost‑sensitive learning (CSL) strategy.
\textcolor{red}{There are two repetitive expressions of the same phrase in this context.}\\
\textcolor{red}{We adopt} a class‑weighted cross‑entropy objective to enforce higher penalties on false negatives \cite{he2009learning}.  
...
5. Section 4.3 contains overly lengthy descriptions of competing models.
Detailed algorithm illustrations for CLAM-SB, DS-MIL, DTFD-MIL, and TransMIL each occupy 4–6 lines. Since these are well-established published methods, their introductions can be appropriately condensed into brief summaries.\\
...
We adopt an image-level stratified 5-fold cross-validation. In addition to standard metrics (AUC and accuracy), we report ROC/PR curves and conduct calibration analysis using reliability diagrams and Brier scores \cite{guo2017calibration}. Unless stated otherwise, results are reported using the final training checkpoint with a default decision threshold $\tau = 0.5$.
...
\textcolor{red}{I think the title here should be revised; otherwise, it is not sufficiently standardized. It is mainly that the notation II* is not sufficiently standard.}
...
To systematically evaluate the impact of cost-sensitive learning on risk control, we extend the baseline SelectiveNet by introducing weights $w_1 \in \{5, 10\}$ on the positive class. This explicitly penalizes \textcolor{red}{false negatives} during training, thereby reducing the high FNR observed in Case~1.
\textcolor{red}{This section should focus on risk rather than false negatives; the current expression is inaccurate.}
\begin{table}[htbp]
    \centering
...
\section{Conclusion}
\textcolor{red}{In my opinion, the conclusion section is relatively underdeveloped. It is necessary to supplement a discussion on the experimental results and clarify the clinical implications of the proposed framework.}
