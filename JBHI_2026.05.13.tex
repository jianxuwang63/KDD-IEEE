...
\textcolor{cyan}{We performed a stratified 5-fold cross-validation at the image level.}\textcolor{magenta}{This part is repeated with the cyan-marked text on Line 324.} In each fold, the ViT‑MIL model was re‑initialized from the same Phikon checkpoint and trained for 15 epochs under identical optimization settings. Evaluation was conducted on the held‑out fold using the last‑epoch model with a fixed threshold $\tau = 0.5$. 
