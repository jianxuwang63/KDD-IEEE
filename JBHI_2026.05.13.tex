...
\textcolor{cyan}{Lung adenocarcinoma (LUAD) is the most common histological subtype of lung cancer, the leading cause of cancer mortality worldwide\cite{bray2024global,luo2025estimated}. Among LUAD variants, the micropapillary (MIP) growth pattern is particularly clinically consequential due to its aggressive nature\cite{wang2020both,travis20152015}. Studies have shown that even a minor MIP component (e.g., $>5\%$) is strongly associated with lymph node metastasis and poor postoperative outcomes \cite{caso2020underlying, dai2017tumor}. As a result, accurately detecting MIP is not only a diagnostic necessity but also a decision-critical factor in surgical planning—such as opting for lobectomy over sublobar resection—and in determining subsequent treatment intensity \cite{dai2017tumor}. Compounding this challenge, the MIP histological pattern accounts for only a small proportion of lung adenocarcinomas \cite{emoto2019expansion}, which renders the corresponding detection task an inherently imbalanced classification problem. In this scenario, conventional models optimized merely for overall accuracy tend to yield high false-negative rates. Such errors are extremely detrimental in cancer diagnosis, as missed positive cases will directly lead to insufficient clinical treatment \cite{araf2024cost}.In this paper, we focus on the challenging task of identifying high-risk MIP cases from pathological local microscopy images, where diagnostic omission could lead to significant undertreatment and adversely affect patient prognosis.}\\
